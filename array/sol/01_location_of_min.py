...
매개변수 nums에 길이가 n인 수열이 주어지면 수열의 원소중에서 가장 작은 값을 찾아 그 값 의 nums 배열의 인덱스 번호를 반환하는 프로그램을 작성하세요.
배열의 인덱스 번호는 0부터 시작합니다.

제한사항:
• nums의 길이 3 <= n <= 100,000
• 배열 nums의 원소는 정수입니다. -1,000,000 <= nums[i] <= 1,000,000 • 배열 nums의 원소는 중복된 값이 존재하지 않습니다.

입력예제 1 설명:
[2, 7, 1, 12, 8, 15, 19]의 최솟값은 1이고 1은 배열의 2번 인덱스 위치에 있습니다.
"""
